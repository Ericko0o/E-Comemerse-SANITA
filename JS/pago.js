...
// Envío del formulario con validaciones y vaciado de carrito
document.getElementById("pago-form").addEventListener("submit", async (e) => {
  ...
  if (
    !/^(0[1-9]|1[0-2])\/\d{2}$/.test(fechaInput) ||
    parseInt(anio, 10) <= 25
  ) {
  ...
  // Simulación de procesamiento + vaciado del carrito
  setTimeout(async () => {
    // 1) Vaciar carrito en el servidor
    await fetch("/vaciar-carrito", {
      method: "DELETE",
      credentials: 'include'
    });

    // 2) Mostrar mensaje final
